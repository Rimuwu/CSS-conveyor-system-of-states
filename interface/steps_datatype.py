...
from interface.utils import func_to_str
...
class StepMessage():
    ...
    def get_text(self, lang: str):
        ...
        if self.translate_message:
            # Тут подразумевается перевод через вашу систему локализации
            return self.text.format(lang, self.text, **self.text_data)
        else:
            return self.text
...
DataType = Union[
    IntStepData, StringStepData, TimeStepData, ConfirmStepData,
    OptionStepData, InlineStepData, CustomStepData, PagesStepData,
    ImageStepData, BaseUpdateType
]
